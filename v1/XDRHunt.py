...
def main(args):
    ...
    try:
        if args.query:
            begin = time.time()
            x = TMVisionOne.XDR(tmconfig.zone, tmconfig.xdr_token, "Trend Micro Vision One Hunt Script Sample")
            ...
            if total_cnt > 0:
                #ret = x.quarantineEmail(js['data']['logs'][0]['msgId'],js['data']['logs'][0]['duser'][0],
                 #                       js['data']['logs'][0]['mailSentTime'])
                result = list()
                if args.filename:
